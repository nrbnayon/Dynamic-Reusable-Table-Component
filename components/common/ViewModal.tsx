...
import { User, X, Calendar, Mail, Building, DollarSign, Shield, Activity } from "lucide-react"
...
export const ViewModal: React.FC<ViewModalProps> = ({
  ...
  const formatValue = (value: unknown, column: ColumnConfig): React.ReactNode => {
    ...
    switch (column.type) {
        ...
        const numValue =
          typeof value === "string"
            ...
            : typeof value === "number"
              ...
              : 0
        return (
          <span className="font-medium text-green-600">{isNaN(numValue) ? "$0" : `$${numValue.toLocaleString()}`}</span>
        )
      ...
      case "date":
        try {
          return <span className="font-medium">{new Date(value as string | number | Date).toLocaleDateString()}</span>
        } catch {
      ...
      case "datetime-local":
        try {
          return <span className="font-medium">{new Date(value as string | number | Date).toLocaleString()}</span>
        } catch {
          return <span className="font-medium">{value.toString()}</span>
        }
      ...
      case "tel":
        return (
          <a href={`tel:${value}`} className="text-blue-600 hover:text-blue-800 font-medium">
            {value.toString()}
          </a>
        )
  ...
  const personalFields = columns.filter((col) => ["name", "email", "avatar", "phone", "address"].includes(col.key))
  const workFields = columns.filter((col) =>
    ["role", "department", "salary", "status", "joinDate", "skills"].includes(col.key),
  )
  const otherFields = columns.filter(
    (col) => !personalFields.includes(col) && !workFields.includes(col) && col.key !== "id",
  )
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <DialogTitle className="flex items-center gap-3">
            {item.avatar && (
              <Avatar className="w-10 h-10">
                <AvatarImage src={item.avatar || "/placeholder.svg"} alt={item.name || "User"} />
                <AvatarFallback>
            ...
            <div>
              ...
              {item.name && <div className="text-lg text-gray-600">{item.name}</div>}
            </div>
          </DialogTitle>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {personalFields.map((column) => {
                  ...
                  if (column.key === "avatar" || value === undefined) return null
          ...
          {(personalFields.length > 0 || workFields.length > 0) && otherFields.length > 0 && <Separator />}
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-mono bg-white px-2 py-1 rounded border">{item.id}</span>
                </div>
                <div className="flex justify-between items-center">
