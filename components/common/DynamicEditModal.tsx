...
import type { GenericDataItem, FormFieldConfig, EditModalConfig } from "../../types/dynamicTableTypes"
...
export const DynamicEditModal: React.FC<DynamicEditModalProps> = ({
  ...
  const validateForm = (): boolean => {
    ...
    fieldConfigs.forEach((config) => {
      ...
      if (config.validation) {
        const { min, max, minLength, maxLength, pattern, custom } = config.validation

        if (typeof value === "string") {
          if (minLength && value.length < minLength) {
            newErrors[config.key] = `${config.label} must be at least ${minLength} characters`
          }
          if (maxLength && value.length > maxLength) {
            newErrors[config.key] = `${config.label} must be no more than ${maxLength} characters`
          }
        ...
        if (typeof value === "number") {
          ...
          if (max !== undefined && value > max) {
            newErrors[config.key] = `${config.label} must be no more than ${max}`
          }
        }
  ...
  const renderFormField = (config: FormFieldConfig) => {
    ...
    const renderInput = () => {
      switch (config.type) {
        ...
        case "select":
          return (
            <Select value={value?.toString() || ""} onValueChange={(val) => handleInputChange(config.key, val)}>
              <SelectTrigger
                id={fieldId}
                className={cn(hasError && "border-red-500 focus:border-red-500", config.className)}
              >
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger className={cn(hasError && "border-red-500")}>
                  <SelectValue placeholder={config.placeholder || `Add ${config.label}`} />
                </SelectTrigger>
                <SelectContent>
                  {config.options
                ...
                <label htmlFor={fieldId} className="cursor-pointer">
                  <div className="space-y-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-1">{config.placeholder || "Max file size 50MB"}</p>
                    </div>
                  </div>
                </label>
              </div>
          ...
          return (
            <div className="relative">
              ...
              <Input
                ...
                step="0.01"
                value={value?.toString() || ""}
                onChange={(e) => handleInputChange(config.key, Number.parseFloat(e.target.value) || 0)}
                placeholder={config.placeholder}
                className={cn("pl-8", hasError && "border-red-500 focus:border-red-500", config.className)}
              />
            </div>
          )
          ...
          return (
            <div className="relative">
              <Input
                ...
                onChange={(e) => handleInputChange(config.key, Number.parseFloat(e.target.value) || 0)}
                placeholder={config.placeholder}
                className={cn("pr-8", hasError && "border-red-500 focus:border-red-500", config.className)}
              />
  ...
  const fieldsBySection = fieldConfigs.reduce(
    (acc, config) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className={cn("max-h-[90vh] overflow-y-auto", modalWidth[modalConfig.width || "lg"])}>
        <DialogHeader>
          ...
          <DialogDescription>{modalConfig.description || "Update the information below."}</DialogDescription>
        </DialogHeader>

        <div className="space-y-6">
          {Object.entries(fieldsBySection).map(([sectionKey, sectionFields]) => {
            const sectionConfig = modalConfig.sections?.find((s) => s.key === sectionKey)

            return (
              <div key={sectionKey} className={cn("space-y-4", sectionConfig?.className)}>
                {sectionConfig && (
                  <div className="space-y-1">
                    <h3 className="text-lg font-semibold">{sectionConfig.title}</h3>
                    {sectionConfig.description && <p className="text-sm text-gray-600">{sectionConfig.description}</p>}
                  </div>
                )}
                ...
                <div className="grid gap-4">
                  {sectionFields.map((config) => {
                    ...
                    return (
                      ...
                      >
                        <div className={gridClass[config.gridCol || "half"]}>{renderFormField(config)}</div>
                      </div>
                    )
                  })}
                </div>
