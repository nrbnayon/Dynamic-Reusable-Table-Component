...
import { Search, Calendar, ChevronUp, ChevronDown, Download, RefreshCw, User, Trash2 } from "lucide-react"
...
export const DynamicTable: React.FC<DynamicTableProps> = ({
  ...
  const [selectedItem, setSelectedItem] = useState<GenericDataItem | null>(null)
  ...
  const formatValue = useCallback((value: unknown, column: ColumnConfig): string => {
    ...
    if (config.enableSearch && searchTerm) {
      filtered = filtered.filter((item) => {
        ...
        return searchableColumns.some((column) => {
          ...
          return formatValue(value, column).toLowerCase().includes(searchTerm.toLowerCase())
        })
      })
    }
  ...
  const handleModalSave = (updatedItem: GenericDataItem) => {
    const updatedData = localData.map((item) => (item.id === updatedItem.id ? updatedItem : item))
  ...
  const confirmDelete = () => {
    if (deleteConfirm.itemId === "bulk") {
      ...
      const updatedData = localData.filter((item) => !selectedItems.includes(item.id))
      setLocalData(updatedData)
      onDataChange?.(updatedData)
      ...
      const updatedData = localData.filter((item) => item.id !== deleteConfirm.itemId)
      setLocalData(updatedData)
      onDataChange?.(updatedData)
  ...
  const renderCellContent = (item: GenericDataItem, column: ColumnConfig) => {
      ...
      return (
        <div className="flex items-center gap-3">
          <Avatar className="w-8 h-8">
            <AvatarImage src={item.avatar || "/placeholder.svg"} alt={value?.toString() || "User"} />
            <AvatarFallback>
    ...
    return (
      <div className={cn("truncate", column.className)} title={formatValue(value, column)}>
    ...
    <div className={cn("space-y-4", config.className)}>
      ...
      <div className="border rounded-lg overflow-hidden">
        <div className="overflow-x-auto">
          <Table>
            <TableHeader className={cn(config.stickyHeader && "sticky top-0 z-10")}>
              <TableRow className={cn(config.bordered && "border-b")}>
                ...
                {(actions.length > 0 || formFields.length > 0) && <TableHead className="w-12">Actions</TableHead>}
              </TableRow>
            </TableHeader>
            <TableBody>
                    ...
                    {(actions.length > 0 || formFields.length > 0) && (
                      <TableCell>
                        <div className="flex items-center gap-1">
                          ...
                          {formFields.length > 0 && !actions.some((action) => action.key === "edit") && (
                          ...
                          {onItemDelete && !actions.some((action) => action.key === "delete") && (
                            <Button
                              variant="ghost"
                              size="sm"
      ...
      {filteredData.length > 0 && config.enablePagination && (
        <div className="text-sm text-gray-600">
          Showing {startIndex + 1} to {Math.min(startIndex + config.itemsPerPage, filteredData.length)} of{" "}
      ...
      <Dialog
        ...
        onOpenChange={() => setDeleteConfirm({ open: false, itemId: "", itemName: "" })}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeleteConfirm({ open: false, itemId: "", itemName: "" })}>
              Cancel
            </Button>
